...
run() {
  ...
  # ⑭ 卸载目标磁盘上的所有分区和 swap
  # ==========================================================================
  # 如果目标磁盘之前被挂载过（比如之前运行过脚本失败重跑），必须先卸载
  # 否则 fdisk 会报"磁盘正在使用"拒绝分区
  if [[ "$dry_run" = false ]]; then
    log INFO "UNMOUNT DISK: $disk" "$output"
...
install-chroot() {
  ...
  curl -Lf "$installer_url/install_2_chroot.sh" >/mnt/install_2_chroot.sh
